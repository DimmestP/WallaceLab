---
Title: "Testing produce simulated total cell RNA counts by comparing lognormal to negbinomial distribution"
...
```
```{r negbinomial_actual_tot_counts_stanfit}
negbinomial_actual_counts_stanfit = function(stan_data){
stan(model_code = "// -*- mode: C -*-
    ...
    model{
      ...
      alpha ~ normal(2000,200);

      // gamma function chosen for phi
      phi ~ normal(3000,300);

      for(idx in 1:NRNA){
        observed_total_RNA[idx] ~ neg_binomial_2(alpha,phi);
      }
    }
...
```{r run_stan_negbinomial_parameter_calc, dependson="negbinomial_actual_tot_counts_stanfit"}
...
# run stan model
tot_actual_stanfit <- negbinomial_actual_counts_stanfit(experimental_data_tot)
...
negbinomial_mean <- tot_actual_stanfit %>%    
                  rstan::extract() %$% 
                      mean(alpha)
negbinomial_sd <- tot_actual_stanfit %>%    
                  rstan::extract() %$% 
                      mean(phi)
set.seed(102)
simulated_actual_total_RNA_counts <- rnbinom(5000,negbinomial_mean,negbinomial_sd)
write_tsv(data.frame(simulated_actual_total_RNA_counts = simulated_actual_total_RNA_counts), path = "../data_processed/actual_total_negbinomial_check.tsv")
```
