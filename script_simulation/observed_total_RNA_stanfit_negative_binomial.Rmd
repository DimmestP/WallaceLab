...
experimental_data_tot <-  list(NRNA=length(filter(TSP_Count_FracBySample, Condition == "30C")$Tot), 
         NRNA_simulated=length(real_tot_simulated$simulated_actual_total_RNA_counts),
         observed_total_RNA=as.integer(round(filter(TSP_Count_FracBySample, Condition == "30C")$Tot)), actual_total_RNA =as.integer(real_tot_simulated$simulated_actual_total_RNA_counts))
...
negbin_phi <- tot_obs_simulations_parameters %>%    
                  rstan::extract() %$% 
                      mean(phi)
set.seed(102)
simulated_observed_total_RNA_counts <- c(1:5000)
for(idx in 1:2000){
simulated_observed_total_RNA_counts[idx] <- rnbinom(1,mu=negbin_alpha_observed*as.integer(real_tot_simulated$simulated_actual_total_RNA_counts[idx]), size=negbin_phi)
}
