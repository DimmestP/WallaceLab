...
lognorm_mean <- tot_obs_simulations_parameters %>%    
                  rstan::extract() %$% 
                      mean(alpha)
lognorm_sd <- tot_obs_simulations_parameters %>%    
                  rstan::extract() %$% 
                      mean(phi)
set.seed(102)
simulated_actual_total_RNA_counts <- rlnorm(100,lognorm_mean,lognorm_sd)
write_tsv(data.frame(simulated_actual_total_RNA_counts = simulated_actual_total_RNA_counts), path = "../data_processed/actual_total_simulation_data.tsv")
```
